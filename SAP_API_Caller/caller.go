...
func (c *SAPAPICaller) HeaderItem(headerItem *requests.HeaderItem) {
	err := c.callPurchaseOrderSrvAPIRequirementHeaderItem("A_PurchaseOrder", headerItem)
	if err != nil {
		c.log.Error(err)
		return
	}
	err = c.outputter.Send(c.outputQueues[0], map[string]interface{}{"message": headerItem, "function": "PurchaseOrderHeaderItem"})
	if err != nil {
		c.log.Error(err)
		return
	}
